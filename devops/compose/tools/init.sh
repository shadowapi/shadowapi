...
# Check if VITE_ZITADEL_CLIENT_ID already exists in .env
if grep -q "^VITE_ZITADEL_CLIENT_ID=" .env; then
  # Update existing line
  sed -i "s|^VITE_ZITADEL_CLIENT_ID=.*|VITE_ZITADEL_CLIENT_ID=$CLIENT_ID|" .env
else
  # Append new line
  echo "" >> .env
  echo "# Generated by init" >> .env
  echo "VITE_ZITADEL_CLIENT_ID=$CLIENT_ID" >> .env
fi

echo "✓ .env updated with VITE_ZITADEL_CLIENT_ID"
