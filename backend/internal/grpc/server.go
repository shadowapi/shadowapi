...
func Provide(i do.Injector) (*Server, error) {
	...
	log := do.MustInvoke[*slog.Logger](i).With("service", "grpc-server")
	dbp := do.MustInvoke[*pgxpool.Pool](i)

	manager := NewWorkerManager(log)
	...
	workerSvc := NewWorkerService(log, dbp, manager)
...
func (s *Server) Manager() *WorkerManager {
	return s.manager
}
