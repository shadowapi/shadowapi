...
func (h *Handler) PlainLogin(ctx context.Context, email, password string) (string, error) {
	var id pgtype.UUID
	var hash string
	err := h.dbp.QueryRow(ctx, `SELECT uuid, password FROM "user" WHERE email=$1 AND is_enabled`, email).Scan(&id, &hash)
	if err != nil {
...
func (h *Handler) SessionStatus(ctx context.Context) (*api.SessionStatus, error) {
	_, ok := session.GetIdentity(ctx)
	return &api.SessionStatus{Active: ok}, nil
}
