...
import { Button, Flex, Form, Header, Item, Picker, Switch, Text, TextField } from '@adobe/react-spectrum';
...
function CronExpressionInput({ value, onChange, errorMessage }) {
    const parseCron = useCallback((cron) => {
    ...
    return (<>
      <Flex direction="column" gap="size-100">
        ...
        <Picker label="Month" selectedKey={month} onSelectionChange={(key) => setMonth(key.toString())} width="124px">
          {monthOptions.map((opt) => (<Item key={opt.key}>{opt.label}</Item>))}
        </Picker>
        <Picker label="Day of Week" selectedKey={dayOfWeek} onSelectionChange={(key) => setDayOfWeek(key.toString())} width="124px">
          {dayOfWeekOptions.map((opt) => (<Item key={opt.key}>{opt.label}</Item>))}
        </Picker>
      </Flex>
...
export function SchedulerForm({ schedulerUUID }) {
    ...
    if (!isAdd && schedulerQuery.isPending)
        return <></>;
    if (pipelinesQuery.isPending)
        return <></>;
    return (<Flex direction="row" justifyContent="center" height="100vh">
      <Form onSubmit={form.handleSubmit(onSubmit)}>
        <Flex direction="column" width="size-4600" gap="size-100">
          ...
          <Controller name="pipeline_uuid" control={form.control} rules={{ required: 'Pipeline is required' }} render={({ field, fieldState }) => (<Picker label="Pipeline" isRequired selectedKey={field.value} onSelectionChange={(key) => field.onChange(key.toString())} errorMessage={fieldState.error?.message} width="100%">
                {pipelinesQuery.data?.pipelines?.map((pipeline) => <Item key={pipeline.uuid}>
                    <span style={{
                    whiteSpace: 'nowrap',
                    height: '24px',
                    lineHeight: '24px',
          ...
          <Controller name="schedule_type" control={form.control} rules={{ required: 'Schedule type is required' }} render={({ field, fieldState }) => (<Picker label="Schedule Type" isRequired selectedKey={field.value} onSelectionChange={(key) => field.onChange(key.toString())} errorMessage={fieldState.error?.message} width="100%">
                <Item key="cron">Cron</Item>
                <Item key="one_time">One Time</Item>
              </Picker>)}/>
          {scheduleType === 'cron' && (<>
              {/* Cron Templates Picker */}
              <Picker label="Select a Cron Template" placeholder="Pick a scenario" onSelectionChange={(val) => {
                switch (val) {
                    case 'every_10_min':
              ...
              <Controller name="cron_expression" control={form.control} rules={{
                required: 'Cron expression is required',
                validate: (value) => {
                    const parts = value.trim().split(/\s+/);
                    return parts.length === 5 || 'Cron expression must have exactly 5 fields';
                },
            }} render={({ field, fieldState }) => <CronExpressionInput value={field.value || ''} onChange={field.onChange} errorMessage={fieldState.error?.message}/>}/>
            </>)}
          {scheduleType === 'one_time' && (<Controller name="run_at" control={form.control} rules={{ required: 'Run At is required' }} render={({ field, fieldState }) => <TextField label="Run At" isRequired type="datetime-local" width="100%" validationState={fieldState.invalid ? 'invalid' : undefined} errorMessage={fieldState.error?.message} {...field}/>}/>)}
          <Controller name="timezone" control={form.control} rules={{ required: 'Timezone is required' }} render={({ field, fieldState }) => (<Picker label="Timezone" isRequired selectedKey={field.value} onSelectionChange={(key) => field.onChange(key.toString())} errorMessage={fieldState.error?.message} width="100%">
                {timezones.map((tz) => <Item key={tz.value}>{tz.label}</Item>)}
              </Picker>)}/>
          <Controller name="is_enabled" control={form.control} render={({ field }) => (<Switch isSelected={field.value} onChange={field.onChange}>
                Enabled
              </Switch>)}/>
          <Controller name="is_paused" control={form.control} render={({ field }) => (<Switch isSelected={field.value} onChange={field.onChange}>
                Paused
              </Switch>)}/>
          {!isAdd && (<>
              <Controller name="last_run" control={form.control} render={({ field }) => <TextField label="Last Run" isDisabled value={field.value || ''} width="100%"/>}/>
              <Controller name="next_run" control={form.control} render={({ field }) => <TextField label="Next Run" isDisabled value={field.value || ''} width="100%"/>}/>
            </>)}
          <Flex direction="row" gap="size-100" marginTop="size-300" justifyContent="center">
            <Button type="submit" variant="cta">
              {isAdd ? 'Create' : 'Update'}
            </Button>
            {!isAdd && (<Button variant="negative" onPress={onDelete}>
                Delete
              </Button>)}
          </Flex>
        </Flex>
...
//# sourceMappingURL=SchedulerForm.jsx.map