...
import { ActionButton, Badge, Cell, Column, Content, Dialog, DialogTrigger, DropZone, Flex, Form, Heading, Item, Picker, Row, TableBody, TableHeader, TableView, TextField, useDragAndDrop, useListData, View, } from '@adobe/react-spectrum';
...
export const PipelineForm = ({ pipelineUUID, userUUID }) => {
    ...
    const [editingSchedulerUUID, setEditingSchedulerUUID] = useState('add');
    const openScheduler = (uuid) => {
    ...
    useEffect(() => {
        ...
        // Now rebuild
        let idx = 0;
        const addLeaf = (categoryName, label) => {
            idx++;
            const parentId = idx;
    ...
    useEffect(() => {
        if (pipelineQuery.data) {
            ...
            rf.addNodes(pipelineQuery.data.flow.nodes);
            rf.addEdges(pipelineQuery.data.flow.edges);
        }
    ...
    const getItemsCallback = useCallback((keys) => {
        return [...keys].map((key) => {
    ...
    const onDrop = async (event) => {
        if (event.type !== 'drop' || event.dropOperation !== 'copy')
            return;
        if (event.items[0].kind !== 'text')
            return;
    ...
    if (datasourceQuery.isPending && storageQuery.isPending && !isAdd)
        return <></>;
    console.log('PipelineForm!', { datasourceQuery, initialEdges, initialNodes, nodes, edges });
    return (<Flex direction="row" width="100%" height="100%">
        ...
        <Form onSubmit={form.handleSubmit(onSubmit)} aria-label="Update flow">
          <Flex direction="column" rowGap="size-200">
            <Flex direction="row" gap="size-200">
              <Controller name="name" control={form.control} rules={{ required: 'Name is required' }} render={({ field, fieldState }) => (<TextField labelPosition="side" isRequired value={field.value ?? ''} onChange={field.onChange} onBlur={field.onBlur} validationState={fieldState.invalid ? 'invalid' : undefined} errorMessage={fieldState.error?.message} aria-label={`Enter ${field.name} value`}/>)}/>

              <ActionButton aria-label="Save or add pipeline" onPress={() => form.handleSubmit(onSubmit)()}>
                {pipelineUUID !== 'add' ? <SaveFloppy /> : <SaveAsFloppy />}
              </ActionButton>

              <ActionButton onPress={onDelete} aria-label="Delete pipeline" isDisabled={deleteMutation.isPending || pipelineQuery.isFetching || pipelineUUID === 'add'}>
                <Delete />
              </ActionButton>
            </Flex>

            <Controller name="datasource_uuid" control={form.control} rules={{ required: 'Datasource is required' }} render={({ field, fieldState }) => (<Picker label="Data Source" isRequired selectedKey={field.value} onSelectionChange={(key) => field.onChange(key ? key.toString() : '')} errorMessage={fieldState.error?.message} width="100%">
                  {datasourceQuery?.data?.map((datasource) => (<Item key={datasource.uuid} textValue={`${datasource.name} ${datasource.type}`}>
                      <span style={{
                    whiteSpace: 'nowrap',
                    margin: '0 10px',
                    lineHeight: '24px',
            ...
            <Controller name="contact_extractor" control={form.control} rules={{ required: 'Datasource is required' }} render={({ field, fieldState }) => (<Picker label="Contact Extractor" isRequired selectedKey={field.value} onSelectionChange={(key) => field.onChange(key.toString())} errorMessage={fieldState.error?.message} width="100%">
                  <Item key="extractor1">
            ...
            <Controller name="storage_uuid" control={form.control} rules={{ required: 'Storage is required' }} render={({ field, fieldState }) => (<Picker label="Storage" isRequired selectedKey={field.value} onSelectionChange={(key) => field.onChange(key.toString())} errorMessage={fieldState.error?.message} width="100%">
                  {storageQuery?.data?.map((storage) => <Item key={storage.uuid}>
                      <span style={{ whiteSpace: 'nowrap', margin: '0 10px', lineHeight: '24px' }}>
            ...
            <Flex direction="row" gap="size-200">
              <ActionButton onPress={() => { }} aria-label="Run pipeline" isDisabled={deleteMutation.isPending || pipelineQuery.isFetching || pipelineUUID === 'add'}>
              ...
              <ActionButton onPress={() => openScheduler('add')} aria-label="Schedule pipeline" isDisabled={deleteMutation.isPending || pipelineQuery.isFetching || pipelineUUID === 'add'}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    : new Date(item.run_at ?? '').toLocaleString()}
                        ...
                        <Cell>
                          <Flex direction="row" gap="size-75">
                            ...
                            <ActionButton aria-label="Remove" isQuiet onPress={() => deleteSchedulerMutation.mutate(item.uuid)}>
                              <Trash />
                            </ActionButton>
                          </Flex>
                        </Cell>
      ...
      <Flex direction="row" flexGrow={1} overflow="auto">
        ...
        <DropZone isFilled onDrop={onDrop} flexGrow={1} aria-label="Drop pipeline step here" UNSAFE_style={{ padding: 0 }} replaceMessage="Add pipeline step here">
          <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} nodeTypes={nodeTypes} ref={dropZoneRef} fitView>
            <Controls />
...
//# sourceMappingURL=PipelineForm.jsx.map