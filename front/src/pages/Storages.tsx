...
export function Storages() {
  ...
  const typeRender = (type: string) => {
    if (type === "postgres") {
      return <Badge variant="neutral"><Data /> <Text>PostgreSQL</Text></Badge>
    }
    return type
  }
  ...
  return (
    <FullLayout>
      <Flex direction="column" margin="size-500" gap="size-100" minWidth={0} minHeight={0}>
        <ActionButton alignSelf="start" onPress={() => navigate("/storages/add")}><Add /><Text>Add Data Source</Text></ActionButton>
        <TableView
